...
from app.utils.security import verificar_token, exigir_rol
...
def get_roles(token_data: dict = Depends(verificar_token)):
    try:
        return roles_services.listar_roles(token_data)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error interno: {str(e)}")

@router.post('/')
def create_rol(data: dict = Body(...), token_data: dict = Depends(exigir_rol('ADMINISTRADOR'))):
    if not data:
...
@router.put('/{nro_rol}')
def update_rol(nro_rol: int, data: dict = Body(...), token_data: dict = Depends(exigir_rol('ADMINISTRADOR'))):
    if not data:
        raise HTTPException(status_code=400, detail='El cuerpo de la petición está vacío.')
...
def delete_rol(nro_rol: int, token_data: dict = Depends(exigir_rol('ADMINISTRADOR'))):
    try:
